...
#This function creates a list of all the MAE values ​​for a given day and finds the medium (median) value.
def create_set1(sensor_metric):
    df_MAE_ind=sensor_metric["MAE_ind"]
    df_MAE_INT=sensor_metric["MAE_INT"]
    end_list=[]
    ...
    for n_element in range(0,num_elements):
        ...
        for n_col in range(1,5):
            ...
            for n_row in range(0,16):
                n_day_ind=df_MAE_ind.iloc[n_row,n_col][n_element]
                n_day_INT=df_MAE_INT.iloc[n_row,n_col][n_element]
    ...
    #Finally we use sorted to order the numbers from highest to lowest
    ordered_tuple=sorted(end_tuple,key=lambda x: x[1],reverse=True)
...
#2 files (LSTM, GRU, BILSTM, BIGRU - TCN,CNNLSTM,CNNGRU) and 5 sheets (POA1,POA2,Rear1,Rear2,Rear3) 
...
with pd.ExcelWriter("Tesis_Results_group1.xlsx") as writer:
...
with pd.ExcelWriter("Tesis_results_group2.xlsx") as writer2:
